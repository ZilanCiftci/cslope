...
export function useViewport(canvasRef: RefObject<HTMLCanvasElement | null>) {
  ...
  const applyView = useCallback(() => {
    ...
    const margin = Math.max(worldW, worldH) * 0.35 + 20;

    const currentScale = activeScale > 0 ? activeScale : 0;
    const needsRefit = (() => {
      if (currentScale <= 0) return true;
      const left = -w / (2 * currentScale) - activeOffset[0];
      const right = w / (2 * currentScale) - activeOffset[0];
      const top = h / (2 * currentScale) - activeOffset[1];
      const bottom = -h / (2 * currentScale) - activeOffset[1];
      return (
        left > xMin - margin ||
        right < xMax + margin ||
        bottom > yMin - margin ||
